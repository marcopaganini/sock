...
void sockopts(int sockfd, int doall) {
    ...
    if (dontroute) {
        option = 1;
        if (setsockopt
            (sockfd, SOL_SOCKET, SO_DONTROUTE, &option, sizeof(option)) < 0)
    ...
    if (maxseg && udp == 0) {
        ...
        if (setsockopt
            (sockfd, IPPROTO_TCP, TCP_MAXSEG, &maxseg, sizeof(maxseg)) < 0)
    ...
    if (broadcast) {
        option = 1;
        if (setsockopt
            (sockfd, SOL_SOCKET, SO_BROADCAST, &option, sizeof(option)) < 0)
        ...
        if (onesbcast) {
            option = 1;
            if (setsockopt
                (sockfd, IPPROTO_IP, IP_ONESBCAST, &option,
                 sizeof(option)) < 0)
    ...
    if (joinip[0]) {
        ...
        if (setsockopt
            (sockfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &join, sizeof(join)) < 0)
    ...
    if (mcastttl) {
        ...
        if (setsockopt
            (sockfd, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl)) < 0)
    ...
    if (keepalive && doall && udp == 0) {
        option = 1;
        if (setsockopt
            (sockfd, SOL_SOCKET, SO_KEEPALIVE, &option, sizeof(option)) < 0)
    ...
    if (nodelay && doall && udp == 0) {
        option = 1;
        if (setsockopt
            (sockfd, IPPROTO_TCP, TCP_NODELAY, &option, sizeof(option)) < 0)
    ...
    if (linger >= 0 && doall && udp == 0) {
        ...
        if (ling.l_onoff == 0 || ling.l_linger != linger)
            err_quit("SO_LINGER not set (%d, %d)", ling.l_onoff,
                     ling.l_linger);
    ...
    if (recvdstaddr && udp) {
        ...
        option = 1;
        if (setsockopt
            (sockfd, IPPROTO_IP, IP_RECVDSTADDR, &option, sizeof(option)) < 0)
